...
async fn env_is_encrypted_and_round_trips() {
    ...
    env.insert(
        "ZABBIX_URL".to_string(),
        "https://zbx.example.com/api".to_string(),
    );
    env.insert("ZABBIX_TOKEN".to_string(), "s3cr3t-token".to_string());
    instances::replace_env(&pool, &secrets, &inst.id, &env)
        .await
        .unwrap();

    // Values come back for editing...
    let shown = instances::env_for_edit(&pool, &secrets, &inst.id)
        .await
        .unwrap();
    assert_eq!(shown, env);
    ...
    let resolved = instances::resolved_env(&pool, &secrets, &reloaded)
        .await
        .unwrap();
    ...
    instances::replace_env(&pool, &secrets, &inst.id, &smaller)
        .await
        .unwrap();
    let after = instances::env_for_edit(&pool, &secrets, &inst.id)
        .await
        .unwrap();
...
async fn config_file_is_encrypted_and_round_trips() {
    ...
    assert!(instances::config_file_for_edit(&pool, &secrets, &inst.id)
        .await
        .unwrap()
        .is_none());

    let content = "{\"token\": \"s3cr3t-config\"}";
    instances::set_config_file(&pool, &secrets, &inst.id, content)
        .await
        .unwrap();

    // Comes back for editing and resolves for launch.
    assert_eq!(
        instances::config_file_for_edit(&pool, &secrets, &inst.id)
            .await
            .unwrap()
            .as_deref(),
        Some(content)
    ...
    assert_eq!(
        instances::resolved_config_file(&pool, &secrets, &inst.id)
            .await
            .unwrap()
            .as_deref(),
        Some(content)
    ...
    instances::set_config_file(&pool, &secrets, &inst.id, "replaced")
        .await
        .unwrap();
    assert_eq!(
        instances::config_file_for_edit(&pool, &secrets, &inst.id)
    ...
    assert!(instances::resolved_config_file(&pool, &secrets, &inst.id)
        .await
        .unwrap()
        .is_none());
}
...
async fn wrong_user_cannot_access_instance() {
    let pool = pool().await;
    let a = users::create(&pool, "a", "alice", "Alice", false)
        .await
        .unwrap();
    let _b = users::create(&pool, "b", "bob", "Bob", false)
        .await
        .unwrap();
    let inst = instances::create(
        &pool,
    ...
    .await
    .unwrap();
    assert!(instances::get_owned(&pool, &inst.id, "b")
        .await
        .unwrap()
        .is_none());
    assert!(instances::get_owned(&pool, &inst.id, "a")
        .await
        .unwrap()
        .is_some());
}
...
async fn migrates_a_legacy_catalog_instance() {
    ...
    instances::migrate_catalog_instances(&pool, &secrets)
        .await
        .unwrap();
    ...
    let env = instances::env_for_edit(&pool, &secrets, "inst1")
        .await
        .unwrap();
    ...
    instances::migrate_catalog_instances(&pool, &secrets)
        .await
        .unwrap();
    let again = instances::env_for_edit(&pool, &secrets, "inst1")
        .await
        .unwrap();
